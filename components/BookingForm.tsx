...
const BookingForm: React.FC = () => {
  return (
    <section id="booking" className="py-24 px-6 bg-[#121212] relative border-t border-[#4A0404]/30 overflow-hidden">
      {/* Subtle depth sphere */}
      <div className="absolute -bottom-1/4 -right-1/4 w-[500px] h-[500px] bg-[#4A0404]/10 blur-[120px] rounded-full pointer-events-none"></div>
        ...
        <div className="mb-20 reveal opacity-0 translate-y-10 transition-all duration-700">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-8">
            {CONFIG.howItWorks.map((step, idx) => (
              <div key={idx} className={`p-6 md:p-8 bg-[#1A1A1A] border border-[#4A0404]/30 rounded-2xl flex flex-col items-center text-center group hover:border-[#FF5F1F]/40 transition-all shadow-xl ${idx === 2 ? 'sm:col-span-2 md:col-span-1' : ''}`}>
                <div className="w-10 h-10 md:w-12 md:h-12 rounded-full border border-[#FF2C2C]/30 flex items-center justify-center mb-3 md:mb-4 text-[#FF5F1F] font-black group-hover:bg-gradient-to-br group-hover:from-[#FF2C2C] group-hover:to-[#FF5F1F] group-hover:text-white transition-all text-sm">
                  {idx + 1}
                </div>
                <h4 className="text-[#EDEDED] font-black uppercase text-xs md:text-sm mb-1 md:mb-2 group-hover:text-[#FF5F1F] transition-colors">{step.label}</h4>

...
const Skills: React.FC = () => {
  ...
  const getEmbedUrl = (url: string) => {
    ...
    return `${url}${separator}autoplay=1&mute=0&rel=0&modestbranding=1&showinfo=0`;
  };
  ...
  return (
    <section id="style" className="py-24 px-6 bg-[#0E0E0E] overflow-hidden border-t border-white/5">
        ...
        <div className="flex flex-col lg:flex-row items-center justify-between gap-16">
          <div className="w-full lg:w-3/5">
            <h2 className="text-4xl md:text-6xl font-black uppercase italic tracking-tighter mb-8 reveal opacity-0 translate-y-10 transition-all duration-700">
              The <span className="text-[#FF2C2C]">Signature</span> Style
            </h2>
            ...
            <div className="grid grid-cols-2 gap-4 md:gap-6 reveal opacity-0 translate-y-10 transition-all duration-700 delay-200">
              {CONFIG.skills.map((skill, index) => (
                <div key={index} className="flex flex-col sm:flex-row items-center sm:items-center space-y-3 sm:space-y-0 sm:space-x-4 p-4 md:p-5 bg-[#1F1F1F] border border-white/5 hover:border-[#FF2C2C]/50 transition-all duration-300 rounded-xl group hover:-translate-y-1 shadow-xl">
                  <div className="flex-shrink-0 w-8 h-8 md:w-10 md:h-10 rounded-full bg-[#FF2C2C]/10 flex items-center justify-center border border-[#FF2C2C]/30 group-hover:bg-[#FF2C2C] group-hover:shadow-[0_0_15px_rgba(255,44,44,0.5)] transition-all">
                    <svg className="w-4 h-4 md:w-6 md:h-6 text-[#FF2C2C] group-hover:text-white transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="w-full lg:w-2/5 flex justify-center relative reveal opacity-0 translate-y-10 transition-all duration-700 delay-400">
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full bg-[#FF2C2C]/5 blur-[100px] rounded-full" />
            
            <div 
              onClick={openVideo}
              className="relative w-full max-w-[260px] md:max-w-[320px] aspect-[9/16] bg-black rounded-[2rem] md:rounded-[2.5rem] border-[4px] md:border-[8px] border-[#1F1F1F] shadow-2xl overflow-hidden group cursor-pointer hover:border-[#FF2C2C]/40 transition-all"
            >
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent flex items-center justify-center">
                <div className="w-16 h-16 md:w-20 md:h-20 bg-[#FF2C2C] rounded-full flex items-center justify-center shadow-[0_0_40px_rgba(255,44,44,0.6)] group-hover:scale-110 transition-transform animate-cta-pulse">
      ...
      {selectedVideo && (
        <div 
          ...
          onClick={closeModal}
        >
          <div className="w-full max-w-[min(90vw,calc(90vh*9/16))] flex justify-end mb-4">
            <button 
              onClick={closeModal}
